...
class OVSBridge(ovs_lib.OVSBridge):
    def __init__(self, br_name, root_helper, ctrl_rate, ctrl_burst):
        ovs_lib.OVSBridge.__init__(self, br_name, root_helper)
        self.datapath_id = None
        self.ctrl_rate = ctrl_rate
        self.ctrl_burst = ctrl_burst
...
class VifPortSet(object):
    def __init__(self, int_br, janus_rest_client):
        super(VifPortSet, self).__init__()
        self.int_br = int_br
        self.api = janus_rest_client
...
class OVSNeutronOFPJanusAgent(sg_rpc.SecurityGroupAgentRpcCallbackMixin):

    RPC_API_VERSION = '1.1'

    def __init__(self, integ_br, tunnel_ip, ovsdb_ip, ovsdb_port,
                 polling_interval, ctrl_rate, ctrl_burst, openflow_controller, root_helper):
        
        super(OVSNeutronOFPJanusAgent, self).__init__()
        ...
        self._setup_integration_br(root_helper, integ_br, tunnel_ip,
                                   ovsdb_port, ovsdb_ip, ctrl_rate, ctrl_burst, openflow_controller)
    ...
    def _setup_integration_br(self, root_helper, integ_br,
                              tunnel_ip, ovsdb_port, ovsdb_ip, ctrl_rate, ctrl_burst, openflow_controller):
        self.int_br = OVSBridge(integ_br, root_helper, ctrl_rate, ctrl_burst)
        self.int_br.find_datapath_id()
        self.int_br.set_controller(openflow_controller)
        ...
        host, port = rest_api_addr.split(':')
        janus_rest_client = self.api = JanusNetworkDriver(host, port)
        
        #ryu_rest_client = client.OFPClient(rest_api_addr)

        self.vif_ports = VifPortSet(self.int_br, janus_rest_client)
...
def main():
    ...
    ctrl_rate = cfg.CONF.OVS.ctrl_rate
    ctrl_burst = cfg.CONF.OVS.ctrl_burst
    ...
    openflow_controller = cfg.CONF.OVS.openflow_controller
    try:
        agent = OVSNeutronOFPJanusAgent(integ_br, tunnel_ip, ovsdb_ip,
                                      ovsdb_port, polling_interval,ctrl_rate,
                                      ctrl_burst, openflow_controller, root_helper)
