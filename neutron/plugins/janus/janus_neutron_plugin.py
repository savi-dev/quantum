...
class JanusNeutronPluginV2(db_base_plugin_v2.NeutronDbPluginV2,
    ...
    def _create_all_tenant_network(self):
        for net in db_api_v2.network_all_tenant_list():
            self.client.updateNetwork(net.id)
        """
    ...
    def create_network(self, context, network):
        ...
        with session.begin(subtransactions=True):
            ...
            net = super(JanusNeutronPluginV2, self).create_network(context,
                                                                 network)
            self._process_l3_create(context, net, network['network'])

            #tunnel_key = self.tunnel_key.allocate(session, net['id'])
            try:
                self._client_create_network(net['id'], tunnel_key = None)
            except Exception:
    ...
    def update_network(self, context, id, network):
        session = context.session
        with session.begin(subtransactions=True):
            net = super(JanusNeutronPluginV2, self).update_network(context, id,
                                                                 network)
    ...
    def delete_network(self, context, id):
        ...
        with session.begin(subtransactions=True):
            ...
            super(JanusNeutronPluginV2, self).delete_network(context, id)
    ...
    def delete_port(self, context, id, l3_port_check=True):
        ...
        with context.session.begin(subtransactions=True):
            ...
            super(JanusNeutronPluginV2, self).delete_port(context, id)
    ...
    def update_port(self, context, id, port):
        ...
        with session.begin(subtransactions=True):
            original_port = super(JanusNeutronPluginV2, self).get_port(
                context, id)
            updated_port = super(JanusNeutronPluginV2, self).update_port(
                context, id, port)
            self._process_portbindings_create_and_update(context,
                                                         port['port'],
                                                         updated_port)
